...
(async () => {
  ...
  try {
    ...
    await page.locator('#disasterGroup').selectOption('Weather and water disasters');
    ...
    assert.ok(await page.locator('#situationSection').isVisible());
    assert.ok(await page.getByLabel('Can vehicles use the road?').isVisible());
    ...
    assert.match(await page.locator('#evidenceCoordinatesOverlay').innerText(), /Lat 14\.5995124.*Lon 120\.9842195/);
