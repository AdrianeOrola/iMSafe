...
(async () => {
  ...
  try {
    ...
    await page.getByRole('heading', { name: 'Incident command dashboard', exact: true }).waitFor({ timeout: 60000 });
    assert.ok(await page.getByRole('heading', { name: '14-day incident trend', exact: true }).isVisible());
    assert.ok(await page.getByRole('heading', { name: 'Filter the whole dashboard', exact: true }).isVisible());
    assert.ok(await page.getByRole('heading', { name: 'Disaster demand by priority', exact: true }).isVisible());
    assert.ok(await page.getByRole('heading', { name: 'Response performance', exact: true }).isVisible());
    assert.ok(await page.getByRole('heading', { name: 'Active queue aging', exact: true }).isVisible());
    assert.equal(await page.locator('.bi-kpi').count(), 6);
    assert.ok(await page.locator('.bi-combo-chart').isVisible());
    await page.getByLabel('Date range').selectOption('7');
    await page.getByRole('button', { name: 'Apply slicers', exact: true }).click();
    await page.getByRole('heading', { name: '7-day incident trend', exact: true }).waitFor();
    assert.match(page.url(), /period=7/, 'Date filter is reflected in the dashboard URL');
    await page.getByRole('link', { name: 'Reset', exact: true }).click();
    await page.getByRole('heading', { name: '14-day incident trend', exact: true }).waitFor();
    ...
    const emptyQueue = page.getByText('No matching incident reports', { exact: true });
    ...
    await page.getByRole('heading', { name: 'Incident command dashboard', exact: true }).waitFor();
    ...
    await page.getByText('Export data', { exact: true }).click();
    assert.match(await page.locator('.bi-stack-row').first().getAttribute('href'), /disaster=Flood/, 'Disaster visual exposes a cross-filter link');
    assert.match(await page.locator('.bi-area-table a').first().getAttribute('href'), /barangay=Synthetic(?:\+|%20)barangay/, 'Area visual exposes a barangay drill-through filter');
    ...
    console.log('PASS: BI-style incident analytics, slicers, combo chart, operational visuals, queue drill-through, and responsive layouts.');
  } finally {
