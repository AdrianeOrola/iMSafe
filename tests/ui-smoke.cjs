...
(async () => {
  ...
  await page.selectOption('#disasterGroup', 'Earth and ground disasters');
  await page.selectOption('#specificType', 'Earthquake');
  await page.selectOption('#particularType', 'Ground shaking');
  assert.ok(await page.getByLabel('Reported magnitude, if officially known').isVisible());
  await page.fill('#earthquakeMagnitude', '6.4');
  await page.selectOption('#shakingStrength', 'Strong');
  await page.selectOption('#buildingDamage', 'Minor cracks or fallen objects');
  await page.selectOption('#roadBridgeStatus', 'Use with caution');
  await page.check('#aftershocksFelt');
  ...
  assert.equal(await page.locator('#disasterGroup').inputValue(), 'Earth and ground disasters');
  ...
  assert.equal(await page.locator('#earthquakeMagnitude').inputValue(), '6.4');
  assert.equal(await page.locator('#shakingStrength').inputValue(), 'Strong');
  assert.ok(await page.locator('#aftershocksFelt').isChecked());
